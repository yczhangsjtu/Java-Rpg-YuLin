...
public class Astar{
  ...
  private static boolean isSlash(Node from, Node to, int[][] maze){
    ...
    if(direct == RpgConstants.LEFT_DOWN && (from.row >= 0 && from.row < maze.length
		&& from.column >= 0 && from.column < maze[from.row].length
		&& maze[from.row][from.column - 1] == 1) && (from.row + 1 < maze.length && 
		maze[from.row + 1][from.column] == 1)){
      result = true;
    }else if(direct == RpgConstants.RIGHT_DOWN && (
		from.row >= 0 && from.row < maze.length && from.column + 1 >= 0
		&& from.column + 1 < maze[from.row].length && maze[from.row][from.column + 1] == 1)
        && (from.row + 1 < maze[from.row+1].length && from.column >= 0 && from.column <= maze[from.row+1].length
		&& maze[from.row + 1][from.column] == 1)){
      result = true;
    }else if(direct == RpgConstants.LEFT_UP && (from.row >= 0 && from.row < maze.length
		&& from.column - 1 >= 0 && from.column - 1 < maze[from.row].length
		&& maze[from.row][from.column - 1] == 1)
        && (from.row - 1 >= 0 && from.column < maze[from.row-1].length
		&& maze[from.row - 1][from.column] == 1)){
      result = true;
    }else if(direct == RpgConstants.RIGHT_UP && (from.row >= 0 && from.row < maze.length
		&& from.column+1 >= 0 && from.column+1 < maze[from.row].length
		&& maze[from.row][from.column + 1] == 1)
        && (from.row - 1 >= 0 && from.column >= 0 && maze[from.row - 1][from.column] == 1)){
      result = true;
    }
